...
Complex::Complex(const Complex& other)//  онструктор копировани€
{
    st = new char[10];
    r = other.r;
    i = other.i;
...
void Complex::setR(int rl)
{
    r = rl;
    toString();
}
void Complex::setI(int im) 
{
    i = im;
    toString();
}
...
const Complex& Complex:: operator = (const Complex &other) // перегрузка оператора присваивани€
{
    r = other.r;
    i = other.i;
    toString();
...
//оператор делени€
Complex Complex:: operator / (Complex& other) // перегрузка оператора делени€
{
    Complex temp;
...
float Complex:: abs(int r, int i) // ћодуль комплексного числа
{
    float b;
    b = sqrt(r * r + i * i);
    return b;
